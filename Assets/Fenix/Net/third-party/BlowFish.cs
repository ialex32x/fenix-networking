...
namespace Fenix.Net
{
    public class BlowFish
    {
        ...
        public BlowFish(string hexKey, string iv)
        {
            randomSource = new RNGCryptoServiceProvider();
            SetupKey(HexToByte(hexKey));
            IV = HexToByte(iv);
        ...
        public BlowFish(byte[] cipherKey, byte[] iv)
        {
            randomSource = new RNGCryptoServiceProvider();
            SetupKey(cipherKey);
            IV = iv;
        ...
        public string Encrypt_CBC(string pt)
        {
            if (!IVSet)
                SetRandomIV();
            var bytes = Encoding.ASCII.GetBytes(pt);

            return ByteToHex(InitVector) + ByteToHex(Encrypt_CBC(bytes, bytes.Length));
        }
        ...
        public string Decrypt_CBC(string ct)
        {
            ...
            var bytes = HexToByte(ct.Substring(16));

            return Encoding.ASCII.GetString(Decrypt_CBC(bytes, bytes.Length)).Replace("\0", "");
        }
        ...
        /// <returns>Plaintext</returns>
        public byte[] Decrypt_CBC(byte[] ct, int len)
        {
            return Crypt_CBC(ct, len, true);
        }
        ...
        public byte[] Encrypt_CBC(byte[] pt, int len)
        {
            return Crypt_CBC(pt, len, false);
        }
        ...
        public void SetupKey(string cipherKey)
        {
            SetupKey(HexToByte(cipherKey));
        }
        ...
        public void SetupKey(byte[] cipherKey)
        {
            bf_P = SetupP();
            //set up the S blocks
            bf_s0 = SetupS0();
        ...
        /// <param name="decrypt">true to decrypt, false to encrypt</param>
        /// <returns>(En/De)crypted data</returns>
        private byte[] Crypt_CBC(byte[] text, int len, bool decrypt)
        {
            if (!IVSet)
            {
            ...
            var textLength = len;
            int paddedLen = (textLength % 8 == 0 ? textLength : textLength + 8 - (textLength % 8));
            byte[] plainText = new byte[paddedLen];
            Buffer.BlockCopy(text, 0, plainText, 0, textLength);
            byte[] block = new byte[8];
